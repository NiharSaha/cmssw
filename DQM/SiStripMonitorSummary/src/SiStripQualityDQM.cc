...
void SiStripQualityDQM::fillGrandSummaryMEs() {
  ...
  for (int i = 1; i < 5; ++i) {
    ...
    for (int j = 0; j < 4; j++) {
      ...
      ME[j]->setBinLabel(i, binlabel.str().c_str());
    }
  }
  ss << "\n";
  for (int i = 1; i < 4; ++i) {
    ...
    for (int j = 0; j < 4; j++) {
      ...
      ME[j]->setBinLabel(i + 4, binlabel.str().c_str());
    }
  }
  for (int i = 4; i < 7; ++i) {
  ...
  for (int i = 1; i < 7; ++i) {
    ...
    for (int j = 0; j < 4; j++) {
      ...
      ME[j]->setBinLabel(i + 10, binlabel.str().c_str());
    }
  }
  ss << "\n";
  for (int i = 1; i < 10; ++i) {
    ...
    for (int j = 0; j < 4; j++) {
      ...
      ME[j]->setBinLabel(i + 16, binlabel.str().c_str());
    }
  }
  for (int i = 10; i < 19; ++i) {
